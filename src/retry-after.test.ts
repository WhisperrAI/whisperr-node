...
describe("Retry-After on delivery", () => {
  ...
  /** First attempt answers `status` (+ Retry-After), every later one 202. */
  function scripted(status: number, retryAfter?: string): WhisperrFetch {
    calls = 0;
    return async () => {
      calls++;
      if (calls === 1) {
  ...
  async function sendOne(fetchImpl: WhisperrFetch): Promise<{ done: Promise<void> }> {
    const w = new WhisperrClient({ apiKey: "wrk_test", fetch: fetchImpl, flushIntervalMs: 0 });
  ...
  it("waits the Retry-After seconds on a 429 instead of the backoff", async () => {
    const { done } = await sendOne(scripted(429, "5"));
  ...
  it("honors an HTTP-date Retry-After on a 503", async () => {
    const { done } = await sendOne(scripted(503, new Date(Date.now() + 8000).toUTCString()));
  ...
  it("caps an excessive Retry-After", async () => {
    const { done } = await sendOne(scripted(429, "86400"));
  ...
  it("falls back to exponential backoff without a usable header", async () => {
    const { done } = await sendOne(scripted(429, "later"));
  ...
  it("ignores Retry-After on other 5xx", async () => {
    const { done } = await sendOne(scripted(500, "30"));
    await vi.advanceTimersByTimeAsync(2300);
    expect(calls).toBe(2);
    await done;
  });

  it("still works with a fetch that returns no headers at all", async () => {
    calls = 0;
    const { done } = await sendOne(async () => {
      calls++;
      return calls === 1 ? { ok: false, status: 429 } : { ok: true, status: 202 };
    });
    await vi.advanceTimersByTimeAsync(2300);
    expect(calls).toBe(2);
    await done;
